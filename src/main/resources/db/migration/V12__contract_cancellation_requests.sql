-- V12: 취소 요청 도메인 (W7 Phase A')
-- - contract_cancellation_requests: 취소 요청 audit (부분 WORM)
-- - 한 계약에 활성(REQUESTED) 취소요청 1건만 partial UNIQUE
-- - 가능 시점 {SHARED, RECEIVER_SIGNED} 는 entity 검증
-- - contracts.status 에 CANCEL_REQUESTED 추가 (CHECK 갱신)

-- ============================================================
-- contracts.status CHECK 갱신 — CANCEL_REQUESTED 추가
-- ============================================================
...
ALTER TABLE contract_status_logs
    ADD CONSTRAINT chk_contract_status_logs_from
        CHECK (from_status IS NULL OR from_status IN ('IN_PROGRESS', 'DRAFT', 'READY', 'SHARED',
                                                      'REVISION_REQUESTED', 'RECEIVER_SIGNED',
                                                      'CANCEL_REQUESTED', 'SIGNED', 'COMPLETED', 'CANCELLED'));

COMMENT ON COLUMN contracts.status IS 'IN_PROGRESS | DRAFT | READY | SHARED | REVISION_REQUESTED | RECEIVER_SIGNED | CANCEL_REQUESTED | SIGNED | COMPLETED | CANCELLED (W7 취소요청)';
...
COMMENT ON COLUMN contract_cancellation_requests.status IS 'REQUESTED | CONFIRMED';
...
-- ============================================================
-- 부분 WORM trigger
-- - immutable: contract_id / requester_user_id / reason / detail / requester_ip / requested_at
...
COMMENT ON FUNCTION contract_cancellation_requests_worm_protect() IS '부분 WORM — requester_* / contract_id / requested_at immutable + DELETE 차단. status / confirmed_* 는 UPDATE 가능';
