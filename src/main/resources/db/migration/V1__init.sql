...
COMMENT ON COLUMN users.guardian_verified_at IS '보호자 인증 완료 시각 (MINOR만 채워짐, NULL이면 미인증)';
...
COMMENT ON COLUMN identity_verifications.purpose IS 'SIGNUP(본인 가입) | GUARDIAN(보호자 인증) — W4에서 CONTRACT_GUARDIAN 추가 예정';
COMMENT ON COLUMN identity_verifications.subject_user_id IS 'GUARDIAN 인증 시 보호 대상(미성년자) user_id, SIGNUP은 NULL';
COMMENT ON COLUMN identity_verifications.guardian_id IS 'GUARDIAN 인증 시 guardians FK (논리 FK), SIGNUP은 NULL';
COMMENT ON COLUMN identity_verifications.guardian_link_token IS 'GUARDIAN 인증 시 사용된 매칭 토큰 (audit 추적용)';

-- =========================================
-- guardians: 보호자 마스터 (계정 없음, identifier_hash로 동일인 식별)
-- - 보호자는 trana 계정 미보유 (anonymous KYC)
-- - 한 보호자가 여러 미성년자 보호 가능
-- - 마스터 정보(name/birth_date)는 안 저장 — 매 인증마다 identity_verifications에 새로 기록
-- =========================================
...
CREATE INDEX idx_guardians_hash ON guardians (identifier_hash);

COMMENT ON TABLE guardians IS '보호자 마스터 — trana 계정 없음. KYC 정보는 identity_verifications에 매번 기록';
COMMENT ON COLUMN guardians.public_code IS '외부 노출용 식별자 (nanoid 12자)';
COMMENT ON COLUMN guardians.identifier_hash IS 'SHA-256(주민번호 등) — 동일 보호자 검출 + 재로그인';

-- =========================================
-- guardian_relations: 미성년자-보호자 관계 (가입 시 1회 확정)
-- - 계약 시 보호자 인증은 별도 (W4~ identity_verifications.purpose=CONTRACT_GUARDIAN)
-- - 동일 (guardian, minor) 페어는 ACTIVE 중 1개만 (UNIQUE partial index)
-- =========================================
...
CREATE UNIQUE INDEX idx_guardian_relations_active
    ON guardian_relations (guardian_id, minor_user_id)
    WHERE status = 'ACTIVE';

CREATE INDEX idx_guardian_relations_minor ON guardian_relations (minor_user_id);
CREATE INDEX idx_guardian_relations_guardian ON guardian_relations (guardian_id);

COMMENT ON TABLE guardian_relations IS '미성년자-보호자 관계 (가입 시 확정). 계약 보호자 인증은 별개';
COMMENT ON COLUMN guardian_relations.status IS 'ACTIVE | REVOKED — 해지 시 revoked_at 기록';

-- =========================================
-- guardian_links: 보호자 KYC 매칭 토큰
-- - 미성년자가 발급 → 보호자에게 SMS/카톡으로 링크 전송
-- - TTL 3일, 1회 사용 (used_at 마킹 후 재사용 X)
-- - 재발급 시 기존 미사용 토큰은 revoked_at으로 강제 만료
-- =========================================
...
CREATE INDEX idx_guardian_links_token ON guardian_links (token);
CREATE INDEX idx_guardian_links_minor_active
    ON guardian_links (minor_user_id, created_at DESC)
    WHERE used_at IS NULL AND revoked_at IS NULL;
CREATE INDEX idx_guardian_links_expires ON guardian_links (expires_at);

COMMENT ON TABLE guardian_links IS '보호자 KYC 매칭 토큰. TTL 3일, 1회용. 재발급 시 기존 미사용 토큰 revoked 처리';
COMMENT ON COLUMN guardian_links.token IS 'jnanoid 21자 (base62, URL-safe, 추측 불가)';
COMMENT ON COLUMN guardian_links.used_at IS '보호자 KYC 시작 시점 (GET /{token} 호출 X, POST 인증 완료 시 마킹)';
COMMENT ON COLUMN guardian_links.revoked_at IS '재발급으로 인한 강제 만료 시각';
