...
- 미성년 + consentType=null → 400 (명시 필수)
    ...
    @Operation(
        ...
        summary = "계약 본문 부분 수정 (DRAFT / REVISION_REQUESTED 단계)",
        description = """
IN_PROGRESS / DRAFT / REVISION_REQUESTED 3 상태에서 호출 가능. null 인 필드는 변경 없음 (PATCH semantics).

상태별 동작:
- IN_PROGRESS → 필수 필드 (title/price/conditionSummary/conditionDetails/tradingPlatform) 모두 채워지면 자동 DRAFT
- DRAFT → 필수 필드 하나라도 비면 자동 IN_PROGRESS
- REVISION_REQUESTED → status 유지 (수정 완료 후 `POST /reshare` 로 SHARED 직접 전이)
...
- creatorRole 은 한 번만 설정 가능 — 이미 설정된 후 다시 보내면 409
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "409",
                description = "IN_PROGRESS / DRAFT / REVISION_REQUESTED 외 상태이거나, creatorRole 이미 설정됨",
                content = [
                    Content(
                        schema = Schema(implementation = ProblemDetailResponse::class),
                        examples = [
...
- 캐러셀 (모든 사진) 은 `GET /v1/contracts/{publicCode}/attachments` 별 호출
    ...
    @Tag(name = "Contract Lifecycle", description = "전자계약 상태 전이 (markReady / share / revert / status-logs)")
    @Operation(
...
- SHARED 이후 본문 수정 불가 (변경 필요 시 CANCELLED 후 신규 DRAFT)
    ...
    @Operation(
        ...
        description = """
SHARED 상태 계약에 대해 수신자(accept 한 BUYER party)가 필드별 수정 이유 입력 → 생성자 알림톡 발송 + status REVISION_REQUESTED 전이.
...
- 수정 후 markReady → share 재호출 (재 알림톡)
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "403",
                description = "접근 권한 없음 (party 아님 또는 생성자 본인)",
                content = [
                    Content(
                        schema = Schema(implementation = ProblemDetailResponse::class),
                        examples = [
    ...
    @PostMapping("/{publicCode}/revisions")
    fun requestRevision(
        @Parameter(hidden = true) userId: Long,
        @PathVariable publicCode: String,
...
- 409 NOT_SHARED          : 현재 SHARED 상태 아님
    ...
    @Tag(name = "Contract Lifecycle", description = "전자계약 상태 전이 (markReady / share / revert / status-logs)")
    @Operation(
        operationId = "contractReshare",
        summary = "REVISION_REQUESTED → SHARED 재공유 (수정 완료)",
        description = """
REVISION_REQUESTED 상태 계약에서 생성자가 본문 수정 완료 → SHARED 직접 전이 + 수신자에게 알림톡 재발송.

흐름:
1. 수신자가 `/revisions` 호출 → status = REVISION_REQUESTED + 생성자에게 "[Trana] 수정 요청 도착" 알림톡
2. 생성자가 `PATCH /v1/contracts/{publicCode}` 로 본문 수정 (PATCH 는 REVISION_REQUESTED 상태에서도 호출 가능 — status 유지)
3. 생성자가 본 endpoint `POST /reshare` 호출 → PDF v1' 재생성 + status SHARED + 새 invitation token 발급 + 수신자에게 "[Trana] 새 계약서 도착" 알림톡 (기존 1번 템플릿 재사용)
4. 수신자가 카톡 클릭 → acceptInvitation (idempotent, 이미 party 라 그대로) → 새 본문 확인 + 서명 또는 재 수정요청
...
- contracts.status = SHARED, contract_status_logs (REVISION_REQUESTED → SHARED, reason="수정 완료 → 재공유") row INSERT
- contract_invitations 신규 row INSERT (기존 invitation 의 receiverName/receiverPhone 재사용, 새 토큰)
- 수신자 카카오톡 알림톡 1번 템플릿 재발송

후속:
- 수신자 측 수정 요청 다시 가능 (반복 가능)
- 생성자가 "수정 취소" 누르면 기존 `POST /revert` (REVISION_REQUESTED → DRAFT) 흐름도 유효 — frontend 선택
                  """,
    )
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "200",
                description = "재공유 성공 (status=SHARED)",
                content = [
                    Content(
                        schema = Schema(implementation = ContractResponse::class),
                        examples = [ExampleObject(name = "reshared", value = ContractExamples.RESHARE_RESPONSE)],
                    ),
                ],
            ),
            ApiResponse(
    ...
    @PostMapping("/{publicCode}/reshare")
    fun reshare(
        @Parameter(hidden = true) userId: Long,
        @PathVariable publicCode: String,
    ): ContractResponse
...
- 403 CONTRACT_403_NOT_ACCESSIBLE   : 생성자 본인이 자기 계약 accept
  ...
  """,
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "403",
                description = "가입 미완료 또는 생성자 본인 호출",
                content = [
                    Content(
                        ...
                        examples = [
                            ...
                            ExampleObject(
                                name = "생성자 본인이 자기 계약 accept (외부 수신자용 token)",
                                value = ContractExamples.NOT_ACCESSIBLE,
                            ),
                        ],
                    ),
                ],
