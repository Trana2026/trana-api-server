 ...
 *
 * 우선순위:
 * 1. `CF-Connecting-IP` — Cloudflare orange proxy 뒤일 때 (dev-api.trana.kr / prod).
 *    표준 XFF 대신 CF 가 자체 헤더로 실 client IP 전달
 ...
 *
 * 통합 전: [com.trana.common.logging.RequestMdcFilter.resolveClientIp] 가 private 으로 같은 로직 보유
 * (향후 refactor 후보 — CF 우선순위 반영 필요).
 */
@Component
class IpExtractor {
    fun extract(request: HttpServletRequest): String? =
        request.getHeader(HEADER_CF_CONNECTING_IP)?.takeIf { it.isNotBlank() }?.trim()
            ?: request
                .getHeader(HEADER_XFF)
